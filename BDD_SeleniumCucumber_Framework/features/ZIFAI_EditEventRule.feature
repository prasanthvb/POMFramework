#2540 Alerts-> Settings-> Event Rule Processing-> User With Edit Access to Event Rule Processing Tab
@2540_Alerts
Feature: Alerts - Active User With Edit Access to Event Rule Processing Tab
	Testing all the positive and negative Scenarios in ZIF UI Alerts - Active User With Edit Access to Event Rule Processing Tab
	
#Prerequisite
#User must have access to click on Settings in Alerts page
#Needed Credentials with Edit Access to Event Rule Processing Tab
#Needed Credentials with No Access to Copy Event Rule 

Background: Launch the Browser with ZIF UI URL
Given Open chrome browser and enter the ZIF UI url for Edit Event Rule Processing Tab

Scenario: Validate No Access to Create new Rule Page  

Given Login with With Edit Access to Event Rule Processing Tab enabled Email ID and Password
And Navigate to Alerts Page and Click on Alert Settings
Then Verify that Create new rule button is not displayed
And Verify User is in Rule Grid Page
...
Given Login with With Edit Access to Event Rule Processing Tab enabled Email ID and Password
And Navigate to Alerts Page and Click on Alert Settings
When Click on Edit icon for any Rule
...
And Verify the existings selected data are available in the slide window
...
Scenario: Validate Rule grid Copy disabled functionality

Given Login with With Edit Access to Event Rule Processing Tab enabled Email ID and Password
And Navigate to Alerts Page and Click on Alert Settings
And Verify that the Copy Icon is disabled
When Click on Copy Icon
Then Verify the Rule is not copied
